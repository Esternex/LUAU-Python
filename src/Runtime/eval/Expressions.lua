...
local function eval_roblox_binary_expr(lhs, rhs, operator)
	local a = lhs[1]
	local b = rhs[1]
	local ta = typeof(a)
	local tb = typeof(b)
	if operator == "+" then
		if ta == "Vector3" and tb == "Vector3" then
			return Values.mkRoblox(a + b)
		elseif ta == "CFrame" and tb == "Vector3" then
			return Values.mkRoblox(a + b)
		end
	elseif operator == "-" then
		if ta == "Vector3" and tb == "Vector3" then
	...
	elseif operator == "*" then
		if ta == "Vector3" and tb == "Vector3" then
			return Values.mkRoblox(a * b)
		elseif (ta == "Vector3" and tb == "number") or (ta == "number" and tb == "Vector3") then
			return Values.mkRoblox(a * b)
		elseif ta == "CFrame" and tb == "CFrame" then
			return Values.mkRoblox(a * b)
		elseif ta == "CFrame" and tb == "Vector3" then
			return Values.mkRoblox(a * b)
		elseif ta == "Vector3" and tb == "CFrame" then
			return Values.mkRoblox(a * b)
		end
	elseif operator == "/" then
		if ta == "Vector3" and tb == "number" then
			return Values.mkRoblox(a / b)
...
local function evaluate_binary_expr(binop, env)
	...
	if lhsType == "roblox" or rhsType == "roblox" then
		local result = eval_roblox_binary_expr(leftHandSide, rightHandSide, binop.operator)
		if result ~= nil then
			return result
		end
	end
	...
	if binop.operator == "+" then
		...
		elseif lhsType == "string" and rhsType == "string" then
			return eval_string_concat_expr(leftHandSide, rightHandSide)
		end
		raiseTypeError()
	end
...
local function eval_tuple_literal(node, env)
	local parent = require(script.Parent.Parent.Interpreter)
	local items = {}
	for i, item in ipairs(node.items) do
		items[i] = parent.evaluate(item, env)
	end
	return Values.mkTuple(items)
end
...
local function eval_assignment(node, env)
	...
	elseif assigne.kind == "IndexExpr" then
		...
		local index = parent.e  -  Edit
  15:36:08.052  SOURCE|valuate(assigne.index, env)
...
local function eval_call_expr(node, env)
	...
	if node.callee.kind == "MemberExpr" then
		...
		if receiver[2] == "instance" then
			local method = receiver[1].methods[node.callee.property]
			if not method then
...
return {
	...
	eval_tuple_literal = eval_tuple_literal,
